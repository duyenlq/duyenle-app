<template>
    ...
    <div class="mt-2 mb-4 md:mt-4 flex flex-col gap-2 md:gap-4">
        ...
        <div class="flex flex-col">
            ...
            <UTable :rows="menuSetting" :columns="columns">
        ...
        <UModal v-model="modal">
            <div class="p-4 flex flex-col gap-1 md:gap-2 bg-gradient-to-br from-slate-100 via-slate-50 to-slate-200 rounded-md">
                ...
                <div class="flex flex-col gap-2 md:gap-3">
                    ...
                    <div class="w-full felx-col gap-2 md:gap-6">
                        <p class="text !text-[14px] text-main font-semibold">active :</p>
                        <!-- <input type="checkbox" v-model="menuForm.active" :value="menuForm.active" class="w-fit accent-main text-white"> -->
                        <select v-model="menuForm.active" class="w-full mt-1 md:mt-2 outline-none border-b-2 border-slate-500/50 text text-slate-500 bg-transparent py-1.5" :class="{'!text-main': menuForm.active}">
                ...
                <div class="flex gap-2 md:gap-3 mt-2 md:mt-4 justify-end">
                    ...
                    <button v-else @click=" setData(), modal = false" class="font-semibold text-white bg-main rounded-lg p-2 md:px-3 text duration-300 hover:shadow-md shadow-black hover:scale-105">Xác nhận</button>
                </div>
            </div>
        </UModal>
    </div>
...
<script setup lang="ts">
    ...
    const { getMenuSetting, addMenuSetting, updateMenuSetting, deleteMenuSetting } = useSettingStore()
    ...
    const setData = async () => {
        ...
        await useAsyncData('addMenu', ()=> addMenuSetting(menuForm.value))
        getData()
        toggleLoadingModal(false)
    }
    ...
    const deleteData = async ( value: any ) => {
        toggleLoadingModal(true)
        await useAsyncData('addMenu', ()=> deleteMenuSetting(value))
        getData()
        toggleLoadingModal(false)
    }
    ...
    const updateData = async () => {
        toggleLoadingModal(true)
        await useAsyncData('updateMenu', ()=> updateMenuSetting(menuForm.value))
        getData()
        toggleLoadingModal(false)
    }
